...
func TestMetadataPath(t *testing.T) {
	...
	if got := urls.MetadataPath("github"); got != expected {
...
func TestMetadataURL(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			port:     443,
			upstream: "github",
			expected: "https://tailgate.tail1234.ts.net/.well-known/oauth-protected-resource/mcp/github",
		},
		{
			name:     "non default port included",
			fqdn:     "tailgate.tail1234.ts.net",
			port:     8443,
			upstream: "linear",
			expected: "https://tailgate.tail1234.ts.net:8443/.well-known/oauth-protected-resource/mcp/linear",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			urls, err := NewURLs(tc.fqdn, tc.port)
			...
			if got := urls.MetadataURL(tc.upstream); got != tc.expected {
				t.Errorf("expected %s, got %s", tc.expected, got)
			}
		})
	}
