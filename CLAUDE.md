...
- **Introspection auth is tailnet identity.** Any tailnet node may introspect (tsidp resolves the caller with WhoIs), so the verifier's HTTP client must dial through tsnet (`tsnet.Server.HTTPClient`). Introspection over the public internet gets `{"active": false}`.
...
- **Lifetimes:** access and ID tokens 5 minutes, refresh tokens 30 days and single-use. Caching introspection results until `exp` is safe and bounded.
...
- `internal/auth` verify hardening: adversarial corpus against the introspection verifier, plus caching bounded by `exp` (every hostile token denied, fail closed).
...
- `internal/proxy/stdiotransport`: the server side of streamable HTTP over a per-session child: synthesized session IDs keyed to identity, JSON-RPC correlation, per-identity-per-upstream cap, idle reap (cap holds under load, children reaped when idle, unknown session gets 404).
