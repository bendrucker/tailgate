...
- The standalone `GET` stream and `resources/subscribe` are replaced by `subscriptions/listen`, a POST whose response stream stays open. Anything that assumes a POST response completes promptly breaks on it. The server acknowledges with `notifications/subscriptions/acknowledged` as the first message on the stream, and the JSON-RPC response to the listen request is how the server ends the subscription rather than how it opens one. A subscription that never ends never produces one. Servers should emit an SSE comment line as a keep-alive and set `X-Accel-Buffering: no`. There is no resumption, so a broken stream loses its request and the client reissues.
