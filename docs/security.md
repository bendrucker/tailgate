...
That error message names which validation the request failed. Each refusal at `400` describes a protocol mistake in the message the caller itself wrote, so naming it discloses nothing about the upstream, the identity, or tailgate's internals. The wording is the transport's own. The internal error carries a package prefix that belongs in the log and never on the wire. Every other status on that path answers in status text alone, since those report a failure whose detail names the child command, the caller's cap, and other internals.
